...
{
    public class AppUpdateBusinessFacade//:UniversalObject
    {
        dynamic objdynamicWrapper; 
		AppUpdateWrapperColletion objAppUpdateWrapperColletion = new AppUpdateWrapperColletion();
        AppUpdateWrapper objAppUpdateWrapper = new AppUpdateWrapper();
		private static readonly string _module = "Core.Business.BusinessFacade.AppUpdateBusinessFacade";
		public AppUpdateBusinessFacade()
        {
            
        }
        public AppUpdateBusinessFacade(dynamic WrapperType)
        {
        ...
        public dynamic ValidateAPP(AppUpdate ObjEntity)
        {
            try
            {
                if (objAppUpdateWrapper.ValidateAPP(ObjEntity))
                {
                    return objAppUpdateWrapper.objWrapperClass;
                }
            }
            catch (Exception ex)
            {
                Log.WriteLog(_module, "ValidateAPP(App_Update=" + ObjEntity + ")", ex.Source, ex.Message, ex);
            }
            finally { }
