    ...
    public class Email
    {
        ...
        public bool Send()
        {
            ...
            try
            {
                ...
                if (ConstantsCommon.UseSmtpCredentials.ToLower().Equals("true"))   // if false then use basic credentials
                {
                    smtpClient.Host = ConstantsCommon.SmtpHost;
                    smtpClient.Port = Convert.ToInt32(ConstantsCommon.SmtpPort);
                    smtpClient.UseDefaultCredentials = ConstantsCommon.UseDefaultCredentials.ToLower().Equals("false") ? false : true;
                    smtpClient.Credentials = new NetworkCredential(ConstantsCommon.SmtpUsername, ConstantsCommon.SmtpPassword); ;
                }
                else
                {
                    smtpClient.Host = ConstantsCommon.DefaultHost;
                }

                smtpClient.EnableSsl = ConstantsCommon.EnableSsl.ToLower().Equals("true") ? true : false;
